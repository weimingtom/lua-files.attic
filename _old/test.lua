...
function iscallable(v)
	return type(v)=='function' or (getmetatable(v)~=nil and getmetatable(v).__call ~= nil)
end
function isindexable(v)
	return type(v)=='table' or (getmetatable(v)~=nil and getmetatable(v).__index ~= nil)
end

isnumeric = isnot(tonumber,nil)

function isnotnil(v) return v ~= nil end
function isnotempty(v) return v ~= '' end
...
function ispositive(n) return type(n)=='number' and n >= 0 end
function isnegative(n) return type(n)=='number' and n < 0 end
