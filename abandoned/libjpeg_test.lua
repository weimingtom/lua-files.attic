...
for _,filename in ipairs(dir('media/jpeg/*.jpg')) do
	...
	assert(libjpeg.load({path = filename}))
	assert(libjpeg.load({cdata = cdata, size = #s}))
	assert(libjpeg.load({string = s}))
	assert(libjpeg.load({cdata_source = read_cdata}))
	assert(libjpeg.load({string_source = read_string}))
